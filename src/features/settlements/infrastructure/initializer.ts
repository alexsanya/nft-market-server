...
export class SettlementDataInitializer {
	...
	public static async populateSettlements(client: RedisClientType): Promise<void> {
		await client.json.set(`settlement:${CreateListingDto.create(SETTLEMENT_ONE.bid.listing).hash(DOMAIN_SEPARATORS[SEPOLIA_ID])}`, '.', SETTLEMENT_ONE);
	}
}
