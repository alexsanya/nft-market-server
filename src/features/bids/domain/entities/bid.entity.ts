...
export class BidEntity {
	...
	public static fromJson(obj: Record<string, unknown>): BidEntity {
		...
		if (!listing) {
			throw AppError.badRequest('This entity requires a listing', [
				{ constraint: 'listing is required', fields: ['listing'] }
			]);
		}
		if (!isAddress(bidder)) {
			throw AppError.badRequest('This entity requires a bider', [
				{ constraint: 'bider is required and must be an address', fields: ['bider'] }
			]);
		}
		if (!isAddress(tokenAddress)) {
			throw AppError.badRequest('This entity requires tokenAddress', [
				{ constraint: 'tokenAddress is required and must be an address', fields: ['tokenAddress'] }
			]);
		}
		if (typeof validUntil === 'undefined') {
			throw AppError.badRequest('This entity requires a validUntil', [
				{ constraint: 'validUntil is required', fields: ['validUntil'] }
			]);
		}
		if (typeof value === 'undefined') {
			throw AppError.badRequest('This entity requires a value', [
				{ constraint: 'value is required', fields: ['value'] }
			]);
		}
		if (!signature) {
